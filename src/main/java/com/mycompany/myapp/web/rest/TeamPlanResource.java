...
public class TeamPlanResource {
    ...
    public ResponseEntity<TeamPlan> partialUpdateTeamPlan(
        ...
        Optional<TeamPlan> result = teamPlanRepository
            .findById(teamPlan.getId())
            .map(existingTeamPlan -> {
                if (teamPlan.getAvailability() != null) {
                    existingTeamPlan.setAvailability(teamPlan.getAvailability());
                }

                return existingTeamPlan;
            })
            .map(teamPlanRepository::save);
